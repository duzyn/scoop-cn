...
Get-ChildItem -Recurse -Path .\bucket | ForEach-Object -Process {
    ...
    # DBeaver
    (Get-Content $_.FullName) -replace 'dbeaver\.io/files', 'gh-proxy.com/https://github.com/dbeaver/dbeaver/releases/download' | Set-Content -Path $_.FullName
    # Or
    ...
    # OBS Studio
    (Get-Content $_.FullName) -replace 'cdn-fastly\.obsproject\.com/downloads/OBS-Studio-(.+)\.zip', 'gh-proxy.com/https://github.com/obsproject/obs-studio/releases/download/$1/OBS-Studio-$1.zip' | Set-Content -Path $_.FullName
    # Or
    ...
    # FastCopy
    (Get-Content $_.FullName) -replace 'fastcopy\.jp/archive', 'gh-proxy.com/https://raw.githubusercontent.com/FastCopyLab/FastCopyDist2/main' | Set-Content -Path $_.FullName
